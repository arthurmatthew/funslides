...
export class Quiz {
    ...
    joinGame(userName: string, gameId: number) {
        ...
        let users: string[] = [];
        get(child(dbRef, `games/${gameId}/users`))
            .then((snapshot) => {
                if (snapshot.exists()) {
                    users = snapshot.val();
                } else {
    ...
    getQuestions() {
        const dbRef = ref(getDatabase());
        let questions: never[] = [];
        get(child(dbRef, `games/${this.gameId}/questionList`))
            .then((snapshot) => {
                if (snapshot.exists()) {
    ...
    getqOn() {
        const dbRef = ref(getDatabase());
        let qOn: number = 0;
        get(child(dbRef, `games/${this.gameId}/qOn`))
            .then((snapshot) => {
                if (snapshot.exists()) {
                    qOn = snapshot.val();
                } else {
    ...
    getqStart() {
        const dbRef = ref(getDatabase());
        let start = 0;
        get(child(dbRef, `games/${this.gameId}/start`))
            .then((snapshot) => {
                if (snapshot.exists()) {
                    start = snapshot.val();
                } else {
    ...
    calcScore(anws: number) {
        ...
        if (
            questions[qOn][anws][1] == true &&
            Date.now() - this.getqStart() < 30000
        ) {
            this.score +=
                1000 * (30000 - (Date.now() - this.getqStart()) / 30000);
        }
    ...
    getScore(pos: number) {
        const dbRef = ref(getDatabase());
        let users: string[] = [];
        get(child(dbRef, `games/${this.gameId}/users`))
            .then((snapshot) => {
                if (snapshot.exists()) {
                    users = snapshot.val();
                } else {
        ...
        let score = null;
        get(child(dbRef, `games/${this.gameId}/users/${users[pos]}/score`))
            .then((snapshot) => {
                if (snapshot.exists()) {
                    score = snapshot.val();
                } else {
